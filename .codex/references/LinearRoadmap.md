...
Use this procedure only when the task requests public roadmap content derived from Linear. Read the team identity in [`AGENT-REPO-CONTEXT.md`](../../AGENT-REPO-CONTEXT.md) and repository rules in [`AGENTS.md`](../../AGENTS.md). Roadmap work is read-only and does not require unrelated implementation work. No repository public roadmap is currently declared by this procedure; current team issues remain the source of truth for state, scope, and sequencing.

## Current pending snapshot

1. Verify the consuming app user, Venworks workspace, and canonical Venworks Canvas team through the same Linear connection. Query the complete current team issue inventory, following every page, rather than assuming a Linear project exists.
