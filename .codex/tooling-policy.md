...
This repository policy applies only to this checkout of `venworks-canvas`, as identified by its existing repository context and reviewed origin. Resolve it with the adopted shared policies using AgentKit's tooling-and-credentials procedure. Repository entries replace shared entries with the same ID in full. Unspecified services keep their existing authorized workflow.
