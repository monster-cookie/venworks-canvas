...
This repository now contains the first production runtime slice: CanvasHost owns a bounded local resource loader, strict BOM-free UTF-8 decoder, iterative HTML tokenizer, and iterative document parser. Component Gallery is a separate plugin whose adapter registers only a consumer-owned entry document; it neither links nor executes the Canvas parser. Its packaged gallery document places escaped source examples before the corresponding declarative controls. Additional packaged fixtures are retained and package-verified as inputs for a future Canvas-owned runtime harness; the current adapter does not execute them. CSS and SVG parser selection, composition, styles, layout, bindings, rendering, transaction commit, and native PC/PS5 acceptance remain unimplemented or unverified, so the complete `VWCANVAS_HTML/2` runtime is not yet conformant. Corpus results remain contract-integrity evidence rather than recorded runtime passes.

## Input acquisition and parser ownership

Every runtime `.html`, `.css`, and `.svg` resource is acquired as an ordered byte sequence. The source-file and aggregate byte limits are enforced on those bytes before a strict UTF-8 decoder rejects a byte-order mark or the first invalid sequence. Successful decoding produces plain `String` text only. The current loader selects the Canvas-owned HTML document parser for `.html` text and retains `.css` and `.svg` text for their future Canvas-owned parsers; it never converts any resource to XML. The document parser identifies template and composition nodes during its single iterative pass and does not reparse them. Future composition will clone already-parsed template children for `vw-use`; standalone fragment resources and a separate fragment parser remain unsupported.
