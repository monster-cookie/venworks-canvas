...
8. Exit the game, enable Component Gallery as well, and repeat loading and ping from the preserved pre-Canvas save. The Example panel must still load and receive `PlayerData`. Component Gallery contributes no parser or presentation code of its own: its adapter registers `index.html`, and CanvasHost must report `HTML PARSED beef70b2-024e-4e9b-a8d5-70a0c882c431 | RESOURCES 3` before it reports that consumer ready. A load or parse error must reject only the gallery candidate and leave the other consumer responsive. This gate proves the archive-only document, stylesheet, and SVG were acquired as text and the HTML was parsed by Canvas; it does not prove CSS, layout, bindings, or rendering, which remain later tasks.
