...
This document explains how existing consumers coexist with the version 2 Canvas host and how maintainers should upgrade, roll back, and diagnose consumer compatibility. Canvas is an independent host and build unit; VWHUD may build on Canvas, while Canvas does not require or link to VWHUD. It complements the field-level [consumer descriptor contract](consumer-descriptor-contract.md) and the independent [HTML Engine 2.0 contract](html-engine-contract.md).
...
| HTML document contract | Version 1 consumers do not register an HTML document. | A version 2 consumer may register a `VWCANVAS_HTML/2` entry document. CanvasHost loads its bounded local HTML/CSS/SVG resource graph as binary data, decodes it to plain text, parses the HTML, and rejects that consumer before `READY` on failure. CSS/SVG parsing, style, layout, bindings, and rendering remain later implementation stages. |
...
The loader lifecycle is one-way for each generation: `loading`, `initializing`, `initialized`, `completing`, and `ready`, followed by `unload` when the host retires it. Duplicate completion signals after `completing` or `ready` are inert. Late initialization, provider, error, or timer signals are checked against the current loader and generation before they can change state. Thrown values from the unload lifecycle callback or `dispose()` are caught after membership retirement so the remaining teardown steps proceed. A HUD removal disposes the host and unloads its consumers; reopening the HUD starts new generations. There is no suspension or resume state, and consumers must rebuild local state after a reload.
...
Data and event routing also isolate consumers. Canvas captures one exact provider callback per channel, takes a recipient snapshot for each dispatch, checks the loader identity and generation before invoking a callback, and catches any thrown value per recipient. Removing the final channel consumer invalidates its local subscription, callback identity, and retained snapshot before native `Unsubscribe`; a provider callback rechecks that exact identity after reading its payload and before retaining or dispatching the snapshot. Routing membership is removed before the unload lifecycle callback and `dispose()` call, so an obsolete consumer cannot receive later data or events. A data or event callback may throw any value, including a non-`Error` value, and be reported without stopping other recipients or removing that current membership, while a throwing ready lifecycle callback causes the affected generation to unload. Canvas has no mechanism to cancel a callback that does not return.

The loaded SWFs are compiled mod code trusted by this integration. Fixed callback names and exception containment prevent accidental cross-consumer routing and isolate callback failures, but they do not create a hostile-code sandbox, authenticate a publisher, or provide a callback cancellation mechanism for arbitrary ActionScript. The lifecycle context is a plain capability description; it does not promise an arbitrary host or context handle, timer handle, or callback into Papyrus.

## Event delivery and recovery limits

`EVENT_SUBMITTED` means that Canvas accepted the packet at the native submission site. It does not mean that a consumer received, displayed, or rendered the event. The shared Watch transport is one-way from Papyrus to the UI and lossy, with no UI-to-Papyrus callback, acknowledgement, delivery receipt, event queue, retained event store, automatic retry, or replay after a HUD reload.
...
The current Player HUD host restores the vanilla Watch data subscriptions and then disables Watch presentation and alert animation while retaining the shared data manager for Canvas. This is the data source used by the approved `uiChannels` catalog. A `null` provider snapshot is distinct from no snapshot observed: the former can be retained and delivered, while the latter produces no replay. An empty declaration array means no subscription, while missing or `null` required declaration fields are invalid.

Canvas owns the Player HUD bootstrap patch and the rest of its Canvas variant movie set: `hudmenu.swf`, `hudmenu.gfx`, `hudmenu_lrg.swf`, and `hudmenu_lrg.gfx` bootstrap the host; `playerhudcomponents.swf`, `playerhudcomponents.gfx`, `playerhudcomponents_lrg.swf`, and `playerhudcomponents_lrg.gfx` retain the Watch adapter; `spaceshiphudmenu.swf` and `spaceshiphudmenu_lrg.swf` retain the Ship loader; and `venworkscui.swf` carries the Canvas host movie. The Canvas build and package definitions use the current vanilla Starfield Interface inputs for these patches and do not consume VWHUD source, binaries, helpers, or build output. The intended dependency direction is for VWHUD to build on Canvas, while Canvas runs independently.

HUD movie overlap is an integration boundary. If another package replaces any of the same `HUDMenu`, Player HUD Watch, or Ship HUD loader files, its replacement can overwrite Canvas's patch or be overwritten by it according to package and load order. Separate BA2 ownership does not merge overlapping edits; use a purpose-built merged patch or a verified load order for co-installation, and do not infer universal VWHUD compatibility from the Canvas consumer contract.
...
The current VWHUD workload is represented by 18 distinct approved data channels across 24 request sites. The 32-consumer, 18-channel, 16-topic, 576-channel-membership, 512-topic-membership, and packet field limits are enforced Canvas consumer boundaries. The separate [HTML Engine contract](html-engine-contract.md#limits) defines its initial compatibility and resource-control ceilings, but neither set is a measured performance ceiling or PC/PS5 rendering acceptance.
