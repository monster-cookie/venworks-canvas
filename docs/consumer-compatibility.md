...
| HTML document contract | No production HTML engine is currently loaded by either consumer protocol. | `VWCANVAS_HTML/2` independently fixes the byte-first Canvas-owned parser route, document, style, local-asset, out-of-band binding metadata, failure, transaction, and resource-limit behavior for a future engine implementation. |
