@{
  ...
  RegistrationRuntimeCases = @(
    ...
    @{ Id = 'pc-example-location-clock'; Packages = @('Canvas', 'Example'); Expected = 'The upper-left Canvas-rendered Example displays universal time and local planetary time from LocalEnvironmentData and LocalEnvData_Frequent, with --:-- for local time in a spaceship. Frequent provider callbacks that do not change the displayed minute leave the visible view stable.' }
    @{ Id = 'pc-example-location-clock-reload'; Packages = @('Canvas', 'Example'); Expected = 'After HUD unload/reopen and save reload, the Example HTML view recreates once with current provider values, shows its pending state until a complete Papyrus effect snapshot arrives, and does not show rows from the previous HUD generation.' }
    @{ Id = 'pc-example-effect-view'; Packages = @('Canvas', 'Example'); Expected = 'On a disposable save, record separately the delay from HUD open to the pending HTML view and from Papyrus effect publication to a committed row. Start with an active cataloged buff or debuff and verify the count and row after the S/P/C sequence; apply another cataloged effect and verify it appears after a complete replacement snapshot; cure or expire one and verify its R removal promptly hides it, including while an older snapshot is in flight. With more than eight effects, wait through every six-second page and verify every row is reachable. Reopen the HUD and repeat with Canvas, Example, Component Gallery, and another UI mod that does not replace the same complete HUD files; check one Example view, correct counts, no stale rows, and no lost Gallery view.' }
  ...
  RuntimeCases = @(
    ...
    @{ Id = 'pc-archive-example'; Packages = @('Canvas', 'Example'); Expected = 'Player HUD visibly renders the namespaced Example HTML view in the upper-left with readable Bethesda-font glyphs, universal and local time, pending or empty effect state, counts, and buff/debuff rows from a complete Papyrus snapshot. Repeat across HUD reopen, save/load, and normal/large variants.' }
