@{
  ...
  RegistrationRuntimeCases = @(
    ...
    @{ Id = 'pc-component-gallery-html-parser'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Component Gallery registers index.html without linking CanvasHtml parser code. CanvasHost reports HTML PARSED for the Component Gallery consumer with exactly three resources before READY, proving archive-only HTML, CSS, and SVG acquisition plus HTML parsing through the Canvas-owned engine. CSS evaluation, layout, bindings, and native rendering remain later acceptance gates.' }
