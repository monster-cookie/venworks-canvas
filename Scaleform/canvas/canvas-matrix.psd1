@{
  ...
  RuntimeCases = @(
    ...
    @{ Id = 'pc-archive-collision-and-missing'; DeveloperFixture = 'Tests/Fixtures/Spriggit/Faults'; Preparation = 'Developer-only: assemble the fault YAML into disposable staged ESMs before this runtime case; Starfield consumes the authored ESMs and never executes Spriggit or reads YAML.'; Packages = @('Canvas', 'Example', 'ComponentGallery'); Expected = 'Using ESMs prepared from the developer fault fixture, Example and Component Gallery remain READY while a different owner is rejected for the Example UUID in Papyrus logs and an intentionally missing consumer movie fails independently.' }
