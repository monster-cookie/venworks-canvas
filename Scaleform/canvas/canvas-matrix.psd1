@{
  ...
  RegistrationRuntimeCases = @(
    ...
    @{ Id = 'pc-example-location-clock-reload'; Packages = @('Canvas', 'Example'); Expected = 'After HUD unload/reopen and save reload, the Example clock recreates exactly once with current provider values. The registrar renews its player OnLocationChange registration through its existing HUD menu callback, and no prior event payload is replayed.' }
  ...
  RuntimeCases = @(
    ...
    @{ Id = 'pc-archive-example'; Packages = @('Canvas', 'Example'); Expected = 'Player HUD visibly renders the namespaced Example clock in the upper-left with universal, local, and next solar-transition rows; the solar row remains pending until a coordinate-bearing surface location event is received.' }
