@{
  ...
  RegistrationRuntimeCases = @(
    ...
    @{ Id = 'pc-component-gallery-cheatsheet-rendering'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Component Gallery registers index.html without linking Canvas HTML, CSS, or rendering classes. CanvasHost reports HTML PARSED with exactly five resources, then HTML RENDERED and READY. The visible Gallery shows grouped rows with Tag, Syntax, and Rendered Result columns; each syntax cell shows literal markup while each result cell is a separate Canvas-rendered display tree. The local gallery-icon.svg and inline SVG path render as distinct supported image examples; unsupported raster formats are absent.' }
    @{ Id = 'pc-component-gallery-css-layout'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Canvas loads gallery.css and its local @import of gallery-theme.css, then visibly applies the supported selector cascade, imported theme color, typography, spacing, width, flex rows and columns, gaps, alignment, and clipping. The relative and absolute position example honors left/top placement and z-index so the front layer overlaps above the back layer. Rows remain aligned without overlap or cumulative position drift in both normal and large HUD modes.' }
    @{ Id = 'pc-component-gallery-data-composition'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Canvas renders the Gallery include, template/use, state, repeat, meter, text binding, visibility, and data-vw-format examples from declared HTML and Gallery-owned sample data. Score: {value} resolves visibly to Score: 42, PlayerData callbacks update the displayed snapshot count, repeat produces three Repeated item rows, and the meter fills to 72 percent without stale generated nodes. Dispatching the dotted venworks.canvas.example.ping topic through the Gallery bridge activates the named pingstate exactly once and reveals its state content.' }
    @{ Id = 'pc-component-gallery-navigation'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Mouse-wheel, Up/Down, and Page Up/Page Down input scrolls the Canvas-rendered document through every Gallery section and clamps at both ends. The document remains clipped to the viewport and input does not create duplicate rows or callbacks.' }
    @{ Id = 'pc-component-gallery-reload-disposal'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Ten Player HUD close/open cycles and one save/reload cycle each recreate exactly one complete Gallery with current sample data. Unload removes Gallery input listeners and the Canvas-generated display tree; late callbacks from an earlier generation do not repaint or affect the replacement session.' }
    @{ Id = 'pc-component-gallery-bounded-failure'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'With one controlled invalid text resource at a time, malformed HTML or CSS, a missing local resource, rejected traversal, excessive expansion, and invalid or oversized binding data each produce one bounded sanitized diagnostic, leave no partial successful Gallery tree, do not loop or grow the display list, and recover after restoring the valid resource and reopening the HUD.' }
