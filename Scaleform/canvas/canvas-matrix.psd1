@{
  ...
  RegistrationRuntimeCases = @(
    ...
    @{ Id = 'pc-component-gallery-cheatsheet-rendering'; Packages = @('Canvas', 'ComponentGallery'); Expected = 'Component Gallery registers index.html without linking Canvas HTML, CSS, or rendering classes. CanvasHost reports HTML PARSED with exactly six resources, then HTML RENDERED and READY. The visible Gallery shows grouped rows with Tag, Syntax, and Rendered Result columns; each syntax cell shows literal markup while each result cell is a separate Canvas-rendered display tree. The local gallery-logo.png, local gallery-icon.svg, and inline SVG path render as three distinct examples.' }
