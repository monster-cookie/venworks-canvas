...
Both bundled demonstration consumers request `PlayerData` and `venworks.canvas.example.ping`. The Example displays lowercase `pong` when its event handler receives that exact topic and keeps the result visible through later player-data updates until the movie unloads. Component Gallery is a declarative test bed and example reference: its small adapter registers only `index.html`, while CanvasHost owns resource acquisition, strict UTF-8 decoding, and HTML parsing. The gallery document places the escaped source for each example immediately before the corresponding declarative control. Layout, CSS evaluation, data binding, and native rendering remain assigned to later HTML-engine tasks, so the current loader/parser slice does not claim a rendered gallery. The Example's `ConsolePing` command publishes the custom event through Canvas for an explicit in-game check.
...
The [Canvas HTML Engine 2.0 contract](docs/html-engine-contract.md) defines the exact `VWCANVAS_HTML/2` document, CSS, local-asset, typed-binding, failure, transaction, and resource-limit boundaries. Its version is independent of the consumer descriptor revision, loaded-movie protocol, wire protocol, and negotiated host API. The initial ActionScript implementation supplies the bounded byte-first loader, strict BOM-free UTF-8 decoder, iterative HTML tokenizer, and iterative document parser. Runtime resources become plain `String` values and never enter XML/E4X, `TextField.htmlText`, or Scaleform stylesheet parsing. CSS/SVG parsing and the later composition-to-render pipeline remain separate work; binding schemas are registered out-of-band and `.json` is not a runtime resource format.

The machine-readable contract and conformance corpus cover the complete planned engine, while the current production slice stops after resource-graph loading and HTML parsing. The corpus contains deterministic future-engine results and at-limit/over-limit recipes for every numeric ceiling; its integrity check validates those artifacts independently and does not execute the ActionScript implementation. Native rendering, responsiveness, and PC or PS5 acceptance remain unverified.
...
cgf "Venworks:CanvasExamples:ExampleRegistrar.ConsoleSubscriptionsProbe"
cgf "Venworks:CanvasExamples:ExampleRegistrar.ConsoleRestoreExample"
...
`ExampleRegistrar.ConsoleSubscriptionsProbe` switches the Example descriptor to the diagnostic movie packaged at `Interface\VenworksCanvas\Consumers\venworks.canvas.example.subscriptions-probe\normal.swf` and `large.swf`, then requests a UI load. Close the console and require the visible `PASS 5 / 5 | FAIL 0` and `ALL REGISTRY DIAGNOSTICS PASSED` marker. The suite executes with a fake provider manager inside Scaleform; it validates registry fan-out and cleanup logic but does not establish native Bethesda provider delivery. Run `ExampleRegistrar.ConsoleRestoreExample` afterward to restore the normal Example descriptor. A deferred attempt must be retried if its console status says the input was not retained.
...
- HTML Engine 2.0 now has its first production ActionScript slice: bounded local resource acquisition, strict UTF-8 decoding, and an iterative HTML tokenizer/document parser used for the Component Gallery entry document. Additional packaged fixtures are package-verified inputs for a future Canvas-owned runtime harness; the current adapter does not execute them. CSS/SVG parsing, composition, style, layout, binding, rendering, complete contract conformance, and native PC/PS5 runtime acceptance are not yet claimed.
