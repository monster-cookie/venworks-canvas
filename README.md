...
This diagnostic build deliberately disables the Watch presentation, including its compass/O2/environment display. The Canvas-owned `BottomLeftGroup` patch stops its movie tree and alert timer, detaches its visual children, bypasses its subscriptions, and makes animation entry points inert. It retains the container referenced by HUD layout and the shared data manager; HUD opacity/visibility updates cannot reveal detached Watch visuals. This is isolation, not a repair of normal Watch animations or a production Watch replacement.

The vanilla custom-alert handler treats `sAlertText` as an animation frame label and sound identifier. Canvas packets are not valid Watch frame labels. Canvas now obtains the data-manager class directly from the patched Watch component, verifies that its presentation is disabled, gets the provider, and then subscribes. Vanilla `Subscribe` replays an existing ready provider; `GetDataFromClient` alone does not force delivery. The receiver reads indexed native alert collections without requiring an ActionScript `Array` cast. It accepts at most 256 entries per callback and routes only `canvas.ui.load` packets through the existing strict parser. Callback, accepted-shape, malformed-entry, and payload-error notices are emitted once each per host instance; unrelated vanilla alert text is not logged. `PROVIDER WAITING FOR CLIENT`, `PROVIDER CALLBACK RECEIVED`, `RX LOAD`, and consumer `READY` are distinct observations. Missing Watch patch deployment fails receiver setup explicitly rather than silently falling back to an unverified manager.
...
For a receiver/Watch-only rebuild, reuse the matching existing `-ScriptsDirectory`, `-PluginsDirectory`, and `-ShipMoviesDirectory`; do not recompile Papyrus or regenerate plugins. Build the auxiliary movies and the four Watch variants into fresh output directories, then pass `-MoviesDirectory` and `-WatchMoviesDirectory` to artifact verification, staging, and full verification. `$vanillaInterface` is the user's extracted vanilla Interface directory, never a source-controlled machine path. `buildConsumerDiscoveryWatchMovies.ps1` rejects existing output directories, checks pinned vanilla/output hashes, builds twice through the pinned compiler, and rejects changes to unrelated decompiled Bethesda classes. Its `-EstablishExpectedHashes` mode is for explicit development of a changed patch only: outputs remain unstageable until reviewed hashes are pinned and a normal build succeeds. Reusable tooling and patch definitions live in `Tools` and `Scaleform`, not `.work`.

Use `stageConsumerDiscoveryProbe.ps1 -HostOnly` for this increment. It verifies the unchanged consumer ESMs and every existing consumer BA2 entry against the selected profile, but neither rebuilds nor installs those two packages. A mismatch fails before the Host swap. All three package targets must remain disjoint, and all three packages retain verification evidence. Back up the current Host ESM/BA2 and record junction targets before staging; rollback restores that Host payload only without renaming plugins or recreating junctions. The Host BA2 gains `Interface/playerhudcomponents.swf`, `.gfx`, `_lrg.swf`, and `_lrg.gfx`; permanent ESMs and existing PEX entries stay unchanged.
...
`Tools/testConsumerDiscoveryUiReceive.ps1` uses the existing Node executable to execute extracted ActionScript subscription/receiver bodies with cached-ready and delayed providers, native-style indexed collections, malformed events, disposal, setup cleanup, and bounded diagnostics. It does not run Flash/Scaleform or demonstrate delivery. `Tools/testConsumerDiscoveryPackaging.ps1` also checks Host-only package selection and canonical-key rejection. Run these alongside the guard, UUID, console, and UI-load checks, both `checkRepo.ps1` modes, and full artifact verification.
...
2. Load directly from the main menu, with no console command required. Test Host only first: the Watch must be absent immediately and the host must report `WATCH PRESENTATION DISABLED`, with no consumer commands submitted. A visible Watch or `WATCH PATCH MISSING OR ACTIVE` means stop and check deployment before enabling consumers. Then test Host+A and Host+A+B. A should produce one load submission, and A+B two spaced submissions, followed by their visible panels. Require `PROVIDER CALLBACK RECEIVED`, `RX LOAD`, `LOAD`, then `READY` per consumer. Allow approximately 15 seconds of unpaused gameplay with the console closed; `SUBSCRIBED` alone is not a pass.
