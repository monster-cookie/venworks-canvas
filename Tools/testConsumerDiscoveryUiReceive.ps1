...
$functions = @{}
foreach ($name in @('subscribe', 'onCustomAlertsData', 'receiveNote')) {
...
const UI_LOAD_PREFIX = 'VWC_EVT/1|canvas.ui.load|';
const first = UI_LOAD_PREFIX + 'fixture-a';
const second = UI_LOAD_PREFIX + 'fixture-b';
function makeHost(disabled = true, ready = true) {
  ...
  host.onCustomAlertsData = new Function('param1', 'UI_LOAD_PREFIX', functions.onCustomAlertsData);
  host.callback = value => host.onCustomAlertsData(value, UI_LOAD_PREFIX);
  ...
  host.owner = { BottomLeftGroup_mc: { getCanvasWatchDisabled: () => disabled, getCanvasWatchDataManager: () => manager } };
  host.subscribe = () => new Function('PROVIDER', functions.subscribe).call(host, PROVIDER);
  return {host, manager};
}
let checks = 0;
function check(action) { action(); checks++; }
check(() => { const {host,manager} = makeHost(); host.subscribe(); host.subscribe(); assert.deepEqual(host.received,[first]); assert.equal(manager.subscribes,1); });
check(() => { const {host,manager} = makeHost(true,false); host.subscribe(); assert.equal(host.received.length,0); manager.listener({data:{aAlerts:[{sAlertText:second}]}}); assert.deepEqual(host.received,[second]); });
check(() => { const {host,manager} = makeHost(false); host.subscribe(); assert.equal(manager.subscribes,0); assert(host.logs.some(x=>x.includes('WATCH PATCH MISSING'))); });
...
check(() => { const {host}=makeHost(); host.callback({aAlerts:[null,42,{}, {sAlertText:5}, {sAlertText:first}]}); assert.deepEqual(host.received,[first]); });
check(() => { const {host}=makeHost(); host.callback({aAlerts:[{sAlertText:'vanilla-private-text'},{sAlertText:'VWC_EVT/1|canvas.registry.snapshot|old'},{sAlertText:first}]}); assert.deepEqual(host.received,[first]); assert(!host.logs.join('').includes('vanilla-private-text')); });
check(() => { const {host}=makeHost(); for(let i=0;i<1000;i++) host.callback({aAlerts:[]}); assert.equal(host.logs.length,2); });
...
foreach ($token in @('this.disableCanvasWatch();', 'this.AlertTimer.stop();', 'removeChildAt(0);', 'return BSUIDataManager;', 'return false;')) {
  if (!($insertions.content.InnerText -join "`n").Contains($token)) { throw "Missing Watch isolation contract: $token" }
}
Write-Host 'Watch isolation patch contracts passed; in-game Watch absence and consumer rendering remain manual acceptance.'
