...
function Resolve-CanvasProfile {
  param(
    [Parameter(Mandatory = $true)]
    [hashtable]$Matrix,
  ...
  if ($profileMatches.Count -ne 1) {
    ...
    throw "Unknown canvas profile '$profileKey'. Expected exactly one of: $available."
  }
...
function Get-CanvasWatchMovieEvidence {
  ...
  $definitionPath = Join-Path $RepositoryRoot 'Scaleform\canvas\build\player-hud-watch.build.psd1'
  ...
  if (($inventory -join '|') -cne ($expectedInventory -join '|') -or
      $evidence.Schema -cne 'VWCANVAS_WATCH_BUILD/1' -or $evidence.PinnedOutputs -cne $true -or
